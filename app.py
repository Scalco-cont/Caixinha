...
def lancamento_salvar_rascunho():
    ...
    Permite que o usuário feche o site e retorne com os dados preservados.
    """
    try:
        ...
        with sqlite3.connect(DATABASE) as conn:
            conn.row_factory = sqlite3.Row
            cursor = conn.cursor()
            cursor.execute('SELECT status_lancamento_caixinha FROM empresas WHERE id = ?', (empresa_id,))
            row = cursor.fetchone()
            if not row:
                return jsonify({'status': 'erro', 'mensagem': 'Empresa não encontrada'}), 404

            status_atual = row['status_lancamento_caixinha']
            # Não sobrescreve rascunho se já enviado para aprovação ou conferido
            status_bloqueados = ('pendente_aprovacao', 'conferido', 'finalizado', 'pendente_revisao', 'revisado')
            if status_atual in status_bloqueados:
                return jsonify({'status': 'ignorado', 'mensagem': 'Rascunho ignorado — lançamento já enviado.'}), 200

            # Marca como em_andamento ao salvar o primeiro rascunho
            novo_status = 'em_andamento' if status_atual not in ('em_andamento',) else status_atual

            cursor.execute('''
                UPDATE empresas
                SET dados_lancamento = ?,
                    status_lancamento_caixinha = ?,
                    atualizado_em = CURRENT_TIMESTAMP
                WHERE id = ?
            ''', (json.dumps(campos), novo_status, empresa_id))
            conn.commit()
